...
const addTotalPointsToChildren = async () => {
  ...
  for (const child of derivedTable) {
    ...
    db('Children')
      .where({ ID })
      .update({ Total_Points: totalPoints + sum });
  }
};
...
const updateWinsForChildren = async () => {
  ...
  const winningChildren = children.filter(
    (child) => child.TeamID === winningTeam[0].ID
  );
...
const updateLosesForChildren = async () => {
  const winningTeam = await getWinningTeam();
  const children = await getChildrenWithTeam();
  const losingChildren = children.filter(
    (child) => child.TeamID !== winningTeam[0].ID
  );
...
const getIDs = async (table) => {
  console.log(`${table}`);
  const data = await db.select([`${table}.ID`]).from(`${table}`);
...
const getChildrenWithTeam = async () => {
  const children = await db
    .select(['T.ID as TeamID', 'C.Name', 'C.ID as ChildID'])
    .from('Children as C')
